@if (isset($cross_sell_product) && count($cross_sell_product) > 1)
    <div component-name="rem-product" class="rem-container160 py-50 @@bg">
        <div class="md:flex justify-between items-center pb-6 md:pb-12">
            <h2 class="rem-text-40 montserrat-semibold text-remdark text-center md:text-left flex items-center">
                {{ __('frontend.checkout.you_may_interested_in') }}

            </h2>
            <a href="#" class="hidden md:flex items-center view-all hide-viewall">
                <span class="text-remdark montserrat-bold rem-text-18 underline">{{ __('frontend.checkout.view_all') }}</span>
        ...
        <div class=" product-container" id="hotseller">
            @foreach ($cross_sell_product as $product)
                <div class="product-cardcontainer border border-remDF bannerheight">
                    <a href="{{ route('front.product.detail', $product->code) }}" class="product-cardimg">
                        <div class="relative seller-imagescontainer">
                            ...
                            <img src="{{ asset($product->feature_image ? $product->feature_image : '') }}" alt="" class="mx-auto h-full object-contain">
                        ...
                        <p class="montserrat rem-text-16 text-remdark pt-5 px-3">{{ $product->$locale_name }}
                        </p>
                    </a>

                    <div class="adminInterProductCartForm product-cardcontent pb-7 px-3">
                        <div class="flex justify-between items-center lg:block">
                            ...
                            <div class="justify-between stock-list">
                                <p class="adminInterProductOutOfStock_{{ $product->id }} montserrat-medium text-remdark rem-text-16 hidden lg:flex items-center {{ $product->isSoldOut($product) ? '' : 'invisible' }}">
                                ...
                                <div class="btncontainer text-right">
                                    <p data-itemid="{{ $product->id }}" data-id="product-{{ $product->id }}"
                                        class="adminInterProductCheckCart bg-remred text-white w-11 h-11 flex items-center justify-center rounded-full ml-auto cursor-pointer">
                                        +</p>
                                    <div
                                        ...
                                        <button data-listing="true" data-itemid="{{ $product->id }}" class="adminInterProductCartUpdate" type="button" data-action="decrease">-</button>
                                        <input name="{{ "inter_product_number_".$product->id }}" type="number" min="0" value="0" class="w-full text-center outline-0">
                                        <button data-listing="true" data-itemid="{{ $product->id }}" class="adminInterProductCartUpdate" type="button" data-action="increase">+</button>
                                    </div>

                                    <input type="hidden" name="{{ "inter_product_name_".$product->id }}" value="{{ $product->$locale_name }}" >
                                    <input type="hidden" name="{{ "inter_product_image_".$product->id }}" value="{{ $product->feature_image }}" >
                                    <input type="hidden" name="{{ "inter_product_type_".$product->id }}" value="{{ $product->type }}" >
                                    <input type="hidden" name="{{ "inter_product_min_stock_qty_".$product->id }}" value="{{ $product->min_stock_quantity }}">
                                    <input type="hidden" name="{{ "inter_product_sell_quantity_".$product->id }}" value="{{ $product->sell_quantity }}" >
                                    <input type="hidden" name="{{ "inter_product_sale_price_".$product->id }}" value="{{ $product->sale_price != 0 ? $product->sale_price : $product->original_price }}">
                                </div>
                            </div>
                        </div>
                        <p class="adminInterProductOutOfStock_{{ $product->id }} montserrat-medium text-remdark rem-text-16 flex lg:hidden items-center {{ $product->isSoldOut($product) ? '' : 'invisible' }}">
                            <span class="w-6 h-6 flex items-center justify-center rounded-full border-2 border-[#F79E1B] mr-2 text-[#F79E1B]">i</span>{{ __('home.product.out_of_stock') }}
                        </p>
                    </div>
                </div>
        ...
        @foreach ($cross_sell_product as $mb_product)
            <div id="cart-modal" data-id="product-{{ $mb_product->id }}"
                ...
                <div class="absolute bottom-0 left-0 w-full rounded-tl-[15px] rounded-tr-[15px] bg-white">
                    ...
                    <div class="adminMbInterProductCartForm px-8 pb-6">
                        <p class="text-20 4xl:text-24 font-semibold text-center pb-4">{{ $mb_product->$locale_name }}</p>
                        <div
                            class="increse-decreasecontainer flex items-center mx-auto lg:mr-[unset] border border-remred w-[140px] lg:w-[99px] h-11 rounded-[22px] px-4 ml-auto">
                            <button type="button" data-action="decrease">-</button>
                            <input name="{{ "mb_inter_product_number_".$mb_product->id }}" type="number" min="0" value="0" class="w-full text-center outline-0">
                            <button type="button" data-action="increase">+</button>
                        </div>
                        <input type="hidden" name="{{ "mb_inter_product_name_".$mb_product->id }}" value="{{ $mb_product->$locale_name }}" >
                        <input type="hidden" name="{{ "mb_inter_product_image_".$mb_product->id }}" value="{{ $mb_product->feature_image }}" >
                        <input type="hidden" name="{{ "mb_inter_product_type_".$mb_product->id }}" value="{{ $mb_product->type }}" >
                        <input type="hidden" name="{{ "mb_inter_product_min_stock_qty_".$mb_product->id }}" value="{{ $mb_product->min_stock_quantity }}">
                        <input type="hidden" name="{{ "mb_inter_product_sell_quantity_".$mb_product->id }}" value="{{ $mb_product->sell_quantity }}" >
                        <input type="hidden" name="{{ "mb_inter_product_sale_price_".$mb_product->id }}" value="{{ $mb_product->sale_price != 0 ? $mb_product->sale_price : $mb_product->original_price }}">
                        <p class="adminMbInterProductOutOfStock_{{ $mb_product->id }} montserrat-medium text-remdark rem-text-16 flex items-center {{ $mb_product->isSoldOut($mb_product) ? '' : 'invisible' }}">
                            <span class="w-6 h-6 flex items-center justify-center rounded-full border-2 border-[#F79E1B] mr-2 text-[#F79E1B]">i</span>{{ __('home.product.out_of_stock') }}
                        </p>
                        <button type="button" data-itemid={{ $mb_product->id }}
                            class="adminMbInterProductCartUpdate bg-rembrown hover:bg-transparent text-mainyellow hover:text-rembrown border border-rembrown w-full py-3 mt-4 montserrat-bold rem-text-14">{{ __('frontend.checkout.add_to_cart') }}</button>
                    </div>
                </div>
            </div>
        @endforeach
...
@endif
@push('scripts')
    <script src="{{ asset('frontend/custom/checkout/interested-product.js') }}" type="text/javascript"></script>
@endpush