            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                            {{ __('backend.sidebar.contact_address') }}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/contact-address/create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i>  {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ url('/admin/contact-address') }}" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-400 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.contact_address.name') }}</th>
                                    ...
                                    @foreach($contactaddress as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/contact-address/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="sticky text-center">
                                                <a href="{{ url('/admin/contact-address/' . $item->id . '/edit') }}" title="Edit ContactAddress">
                                                ...
                                                <form method="POST" action="{{ url('/admin/contact-address' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($contactaddress) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $contactaddress->appends([
