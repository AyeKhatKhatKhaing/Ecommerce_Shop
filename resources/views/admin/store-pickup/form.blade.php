<div class="row">
    <div class="col-md-8">
        <h2 class="fs-1">@if($formMode == "edit") {{ __('backend.common.edit') }} @else {{ __('backend.common.create') }} @endif {{ __('backend.sidebar.store_pickup') }}</h2>
    </div>
                ...
                <div class="tab-content">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-4">
                            {!! Form::label('name',  __('backend.store_pickup.location_type'), ['class' => 'from-label mb-3 required']) !!}
                            {!! Form::select('type',['hk'=>'Hong Kong','ma'=>'Macau'], $formMode=="edit" ? $pickup->type->value : null, ['class' => 'form-select', 'data-control' => 'select2', 'data-hide-search' => "true", 'data-placeholder' => "Select Countr"],['id' => 'location-select']) !!}
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                @foreach (config('locale.langs_code') as $key => $code)
                                    @if ($lngcode == $key)
                                        {!! Form::label('name_'.$lngcode, __('backend.store_pickup.name').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                    @endif
                                @endforeach
