            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                           {{__('backend.sidebar.menu')}}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/menu/create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{__('backend.common.add_new')}}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ url('/admin/menu') }}" id="menu-form" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{__('backend.menu.name')}}</th>
                                        <th class="min-w-150px">{{__('backend.menu.sub_menu')}}</th>
                                        <th class="min-w-100px">{{__('backend.common.enable')}}</th>
                                        <th class="min-w-100px">{{__('backend.common.last_updated')}}</th>
                                        <th class="min-w-100px">{{__('backend.common.actions')}}</th>
                                    </tr>
                                </thead>
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($menu as $item)
                                        <tr>
                                            ...
                                            <td>
                                                {{ isset($item->countries) ? $item->getCountriesName($item->countries) : $item->getPromotionsName($item->promotions) }}
                                            </td>
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/menu/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <form method="POST" action="{{ url('/admin/menu' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($menu) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $menu->appends([
