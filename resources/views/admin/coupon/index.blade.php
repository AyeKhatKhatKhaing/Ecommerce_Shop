            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                            {{  __('backend.sidebar.coupon') }}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/coupon/create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{  __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ url('/admin/coupon') }}" id="coupon-form" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{  __('backend.coupon.code') }}</th>
                                        <th class="min-w-150px">{{  __('backend.coupon.description') }}</th>
                                        <th class="min-w-150px">{{  __('backend.coupon.usage_per_coupon') }}</th>
                                        <th class="min-w-150px">{{  __('backend.coupon.limit_per_user') }}</th>
                                        <th class="min-w-100px">{{  __('backend.common.enable') }}</th>
                                        <th class="min-w-150px">{{  __('backend.coupon.expiry_date') }}</th>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($coupon as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->code }}</td>
                                            <td>{{ isset($item->descriptions) && isset($item->descriptions['hant']) ? $item->descriptions['hant'] : ''  }}</td>
                                            <td>{{ $item->per_coupon_usage }} / {{ $item->per_coupon }}</td>
                                            <td>{{ $item->per_user }}</td>
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/coupon/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="sticky text-center">
                                                <a href="{{ url('/admin/coupon/' . $item->id . '/edit') }}" title="Edit Coupon">
                                                ...
                                                <form method="POST" action="{{ url('/admin/coupon' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($coupon) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $coupon->appends([
