        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header border-0">
                    <h3 class="card-title">
                        {{ __('backend.countries.countries_list') }}
                    </h3>
                    <div class="card-toolbar">
                        <a href="{{ url('/admin/member-country/create') }}" class="btn btn-primary btn-hover me-2" title="Add New Country">
                            <i class="fa fa-plus" aria-hidden="true"></i> {{ __('backend.common.add_new') }}
                        </a>
                    </div>
                </div>
                <form method="get" action="{{ route('member-country.index') }}" id="country-form" class="filter-clear-form">
                    <div class="card-header border-0">
                ...
                <div class="card-body pt-0">
                        ...
                        <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                            <thead>
                                <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                    <th>#</th>
                                    <th>{{ __('backend.countries.country_name') }}</th>
                                    <th>{{ __('backend.common.enable') }}</th>
                                    <th>{{ __('backend.common.last_updated') }}</th>
                                    <th class="text-center sticky">{{ __('backend.common.actions') }}</th>
                                </tr>
                            </thead>
                            <tbody class="fw-bold text-gray-600">
                                @foreach($membercountry as $item)
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/member-country/status-change')"
                                                name="status{{ $item->id }}"
                                                {{ $item->status ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center sticky">
                                        <a href="{{ url('/admin/member-country/' . $item->id . '/edit') }}" title="Edit Member Country">
                                        ...
                                        {!! Form::open([
                                        'method' => 'DELETE',
                                        'url' => ['/admin/member-country', $item->id],
                    ...
                    <div class="row my-3">
                        <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                            {{ Admin::tableLength($membercountry) }}
                        </div>
                        <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                            <div class="pagination-wrapper"> {!! $membercountry->appends([
