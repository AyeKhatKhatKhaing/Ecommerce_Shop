        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header border-0">
                    <h3 class="card-title">
                        {{ __('backend.attributes.attributes_list') }}
                    </h3>
                    <div class="card-toolbar">
                        <a href="{{ url('/admin/product-attribute/create') }}" class="btn btn-primary btn-hover me-2" title="Add New Category">
                            <i class="fa fa-plus" aria-hidden="true"></i> {{ __('backend.common.add_new') }}
                        </a>
                    </div>
                </div>
                <form method="get" action="{{ route('product-attribute.index') }}" id="attribute-form" class="filter-clear-form">
                    <div class="card-header border-0">
                ...
                <div class="card-body">
                        ...
                        <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                            <thead>
                                <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                    <th>#</th>
                                    <th class="min-w-150px">{{ __('backend.attributes.attribute') }}</th>
                                    <th class="min-w-250px">{{ __('backend.attributes.values') }}</th>
                            ...
                            <tbody class="fw-bold text-gray-600">
                                ...
                                <tr>
                                    ...
                                    <td> 
                                       <div style="max-height:100px; overflow:auto;">{{ $item->attributes ? $item->getAttributeString($item->attributes) : '' }}</div>
                                    </td>
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/product-attribute/status-change')"
                                                name="status{{ $item->id }}"
                                                {{ $item->status ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    ...
                                    <td class="sticky text-center">
                                        <a href="{{ url('/admin/product-attribute/' . $item->id . '/edit') }}" title="Edit Product Attribute">
                                        ...
                                        {!! Form::open([
                                        'method' => 'DELETE',
                                        'url' => ['/admin/product-attribute', $item->id],
                    ...
                    <div class="row my-3">
                        <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                            {{ Admin::tableLength($attribute_terms) }}
                        </div>
                        <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                            <div class="pagination-wrapper"> {!! $attribute_terms->appends([
