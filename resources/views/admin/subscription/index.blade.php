...
@section('title', __('backend.sidebar.subscription'))
@section('breadcrumb', __('backend.sidebar.subscription'))
@section('breadcrumb-info', __('backend.sidebar.subscription'))
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                            {{ __('backend.sidebar.subscription') }}
                        </h3>
                    </div>
                    <form method="get" action="{{ url('/admin/subscription') }}" id="subscription-form" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.subscription.name') }}</th>
                                        <th class="min-w-250px">{{ __('backend.subscription.email') }}</th>
                                        <th class="min-w-150px">{{ __('backend.subscription.is_subscribe') }}</th>
                                        <th class="min-w-150px">{{ __('backend.subscription.subscription_date') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($subscription as $item)
                                        <tr>
                                            ...
                                            @endphp
                                            <td>
                                                {{ isset($item->member)? $item->member->getFullName() : $name[0] }}
                                            </td>
                                            <td>{{ $item->email }}</td>
                                            <td>
                                                @if($item->status == 1)
                                            ...
                                            <td>
                                                <span class="fw-bolder text-gray-600">{{ $item->createUser ? $item->createUser->name : '' }}</span><br>
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($subscription) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $subscription->appends([
