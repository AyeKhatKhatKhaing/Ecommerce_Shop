                ...
                <div class="card">
                    <div class="card-header border-0"> 
                        ...
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/blog-category/create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ url('/admin/blog-category') }}" id="blog-category-form" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($blogcategory as $item)
                                        <tr>
                                            ...
                                            <td>{{ isset($item) && isset($item->names['hant']) ? $item->names['hant'] : '' }}</td>
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/blog-category/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="sticky text-center">
                                                <a href="{{ url('/admin/blog-category/' . $item->id . '/edit') }}" title="Edit BlogCategory">
                                                ...
                                                <form method="POST" action="{{ url('/admin/blog-category' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($blogcategory) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $blogcategory->appends([
