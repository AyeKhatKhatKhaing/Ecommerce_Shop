                    ...
                    <div class="card-body">
                                    ...
                                    @foreach($blog as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div class="symbol symbol-50px">
                                                        <span class="symbol-label" style="background-image:url({{ asset($item->blog_image) }}); width: 170px; height: 100px;"></span>
                                                    </div>
                                                    <div class="ms-5">
                                                        <div>
                                                            {{ isset($item) && isset($item->titles) ? $item->titles['hant'] : '' }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                {{ isset($item) && isset($item->blog_category) ? $item->blog_category->names['hant'] : '' }}
                                            </td>
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/blog/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="sticky text-center">
                                                ...
                                                <form method="POST" action="{{ url('/admin/blog' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($blog) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $blog->appends([
