<div class="row">
    <div class="col-md-8">
        <h2 class="fs-1">@if($formMode == "edit") {{ __('backend.common.edit') }} @else {{ __('backend.common.create') }} @endif {{ __('backend.sidebar.privacy_policy') }}</h2>
    </div>
        ...
        <div class="form-group">
            <div class="form-group">
                <div class="float-left">
                    ...
                    <button type="button" class="btn btn-secondary cancel" onclick="window.location='{{ url('/admin/privacy-policy') }}'"><i class="bi bi-x"
                            ...
                            <div class="col-md-12">
                                <div class="form-group {{ $errors->has('title_'.$lng) ? 'has-error' : ''}}">
                                    @foreach (config('locale.langs_code') as $key => $code)
                                        @if ($lng == $key)
                                            {!! Form::label('title_'.$lng, __('backend.privacy_policy.title').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                        @endif
                                    @endforeach
                                    {!! Form::text('title_'.$lng, isset($privacypolicy) && isset($privacypolicy->title) ? $privacypolicy->title[$lng] : null, ('' == 'required') ? ['class' => 'form-control editor', 'required' => 'required'] : ['class' => 'form-control editor']) !!}
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="form-group {{ $errors->has('description_'.$lng) ? 'has-error' : ''}}">
                                    @foreach (config('locale.langs_code') as $key => $code)
                                        @if ($lng == $key)
                                            {!! Form::label('description_'.$lng, __('backend.privacy_policy.description').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                        @endif
                                    @endforeach
                                    {!! Form::textarea('description_'.$lng, isset($privacypolicy) && isset($privacypolicy->description) ? $privacypolicy->description[$lng] : null, ('' == 'required') ? ['class' => 'form-control editor', 'required' => 'required'] : ['class' => 'form-control editor']) !!}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group{{ $errors->has('meta_title_'.$lng) ? 'has-error' : ''}}">
                                                    @foreach (config('locale.langs_code') as $key => $code)
                                                        ...
                                                        @endif
                                                    @endforeach
                                                    {!! Form::text('meta_title_'.$lng, isset($privacypolicy) && isset($privacypolicy->meta_titles) ? $privacypolicy->meta_titles[$lng] : null, ('' == 'required') ? ['class' => 'form-control', 'required' => 'required'] : ['class' => 'form-control']) !!}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group{{ $errors->has('meta_description_'.$lng) ? 'has-error' : ''}}">
                                                    @foreach (config('locale.langs_code') as $key => $code)
                                                        ...
                                                        @endif
                                                    @endforeach
                                                    {!! Form::textarea('meta_description_'.$lng, isset($privacypolicy) && isset($privacypolicy->meta_descriptions) ? $privacypolicy->meta_descriptions[$lng] : null, ('' == 'required') ? ['class' => 'form-control h-150px', 'required' => 'required'] : ['class' => 'form-control h-150px']) !!}
                                            ...
                                            <div class="card-body pt-0">
                                                ...
                                                <div class="panel-block">
                                                    ...
                                                    <div id="holder-meta-image">
                                                            @if(!empty($privacypolicy->meta_image))
                                                                <div class='lfmimage-container meta-imagelfmc0'>
                                                                    <img src="{{ asset($privacypolicy->meta_image) }}" class='lfmimage w-100' style="height: 20rem;">
                                                                    <div>
                                                    ...
                                                    <div class="input-group mt-3">
                                                            ...
                                                            <input id="meta-image" class="form-control" type="text" name="meta_image" value="{{isset($privacypolicy) ? $privacypolicy->meta_image : ''}}">
                                                    </div>  
                                                    </div>
                                                </div>
                                                <div class="row mt-4">
