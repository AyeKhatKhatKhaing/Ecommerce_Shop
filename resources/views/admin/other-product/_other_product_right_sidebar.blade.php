<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="panel-block">
            <div class="form-group">
               <div id="holder-feature-image">
                    @if(!empty($other_product->feature_image))
                        <div class='lfmimage-container feature-imagelfmc0'>
                            <img src="{{ isset($other_product->feature_image) ? asset($other_product->feature_image) : asset(old('feature_image')) }}" class='lfmimage w-100' style="height: 20rem;">
                            <div>
               ...
               <div class="input-group mt-3">
                    ...
                    <input id="feature-image" class="form-control" type="text" name="feature_image" value="{{isset($other_product) ? $other_product->feature_image : old('feature_image')}}">
               </div>
        ...
        <div class="row mt-4">
            <div class="col-md-12">
                {!! Form::label('feature_image_alt', __('backend.common.feature_image_alt'), ['class' => 'control-label mb-3']) !!}
    ...
    <div class="card-body">
        <div class="d-flex justify-content-between nopadding">
            ...
            <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="is_cross_sell" id="is_cross_sell" {{ isset($other_product) && $other_product->is_cross_sell == 1 ? 'checked' : '' }}>
            </div>
        </div>
    </div>
</div>
<div class="card mt-4">
    <div class="card-body">
        <div class="d-flex justify-content-between nopadding">
            ...
            <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="is_exclusive" id="is_exclusive" {{ isset($other_product) && $other_product->is_exclusive == 1 ? 'checked' : '' }}>
            </div>
        </div>
    </div>
</div>
<div class="card mt-4">
    <div class="card-body">
        <div class="d-flex justify-content-between nopadding">
            <label for="is_other" class="control-label fs-4 fw-bold">{{ __('backend.other_product.is_other') }}</label>
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="is_other" id="is_other" checked>
            </div>
        </div>
    </div>
</div>
<div class="card mt-4">
    <div class="card-header">
        <div class="card-title">
            {!! Form::label('sort', __('backend.other_product.sort'), ['class' => 'control-lbael']) !!}
        </div>
    </div>
    <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="overflow p-3">
                            <div class="form-check form-check-custom form-check-solid mb-5">
                                <input class="form-check-input" name="categories[]" type="checkbox" value="{{ $category->id }}" id="{{ $category->name_en }}" {{ (is_array(old('categories')) && in_array($category->id, old('categories'))) ? ' checked' : '' }} {{ isset($category_array) && in_array($category->id, $category_array) ? 'checked' : ''}}/>
...
<div class="accordion mt-4" id="product-country">
    <div class="accordion-item">
        <h2 class="accordion-header" id="country_header">
            <button class="accordion-button fs-4 fw-bold min-h-75px {{ (isset($other_product) && isset($other_product->country_id) || $errors->has('country_id') || is_array(old('country_id')) ) ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#product_country" aria-expanded="false">
                {{ __('backend.products.country') }}
            </button>
        </h2>
        <div id="product_country" class="accordion-collapse collapse {{ (isset($other_product) && isset($other_product->country_id) || $errors->has('country_id') || is_array(old('country_id')) ) ? 'show' : '' }}" aria-labelledby="country_header" data-bs-parent="#product-country">
            <div class="accordion-body">
                <select class="form-select" name="country_id" id="country-id" data-control="select2" data-hide-search="true" placeholder="Please Select Country" data-placeholder="Please Select Country" data-allow-clear="true">
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" {{ isset($other_product) && $other_product->country_id == $country->id ? 'selected' : '' }}>{{ $country->name_en }}</option>
                    @endforeach
                </select>
                <div class="row mt-4">
                    <div class="col-md-12">
                        <select class="form-select" name="region_id" id="region-id" data-control="select2" data-hide-search="true" placeholder="Please Select Region" data-placeholder="Please Select Region" data-allow-clear="true">
                            @if (isset($region_list) && count($region_list) > 0 && $other_product->region_id)
                                @foreach ($region_list as $region)
                                    <option value="{{ $region->id }}" {{ isset($other_product) && $other_product->region_id == $region->id ? 'selected' : '' }}>{{ $region->name_en }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
...
<div class="accordion mt-4" id="product-offer-promotion">
    <div class="accordion-item">
        <h2 class="accordion-header" id="offer_promotion_header">
            <button class="accordion-button fs-4 fw-bold min-h-75px {{ (isset($other_product->offer_promotion_id) || $errors->has('offer_promotion_id')) ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#product_offer_promotion" aria-expanded="false">
                {{ __('backend.products.offer_promotion') }}
            </button>
        </h2>
        <div id="product_offer_promotion" class="accordion-collapse collapse {{ (isset($other_product->offer_promotion_id) || $errors->has('offer_promotion_id')) ? 'show' : '' }}" aria-labelledby="offer_promotion_header" data-bs-parent="#product-offer-promotion">
            <div class="accordion-body">
                <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="overflow p-3">
                                <div class="form-check form-check-custom form-check-solid">
                                    <input class="form-check-input" name="offer_promotion_id" type="radio" value="{{ $offer_promotion->id }}" {{ isset($other_product) && $other_product->offer_promotion_id == $offer_promotion->id ? 'checked' : '' }}/>
...
<div class="accordion mt-4" id="product-label">
    <div class="accordion-item">
        <h2 class="accordion-header" id="label_header">
            <button class="accordion-button fs-4 fw-bold min-h-75px {{ (isset($other_product->label_id) || $errors->has('label_id')) ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#product_label" aria-expanded="false">
                {{ __('backend.products.product_label') }}
            </button>
        </h2>
        <div id="product_label" class="accordion-collapse collapse {{ (isset($other_product->label_id) || $errors->has('label_id')) ? 'show' : '' }}" aria-labelledby="label_header" data-bs-parent="#product-label">
            <div class="accordion-body">
                <div class="row">
                    @foreach($product_labels as $product_label)
                        <div class="col-md-12">
                            <div class="overflow p-3">
                                <div class="form-check form-check-custom form-check-solid">
                                    <input class="form-check-input" name="label_id" type="radio" value="{{ $product_label->id }}" {{ isset($other_product) && $other_product->label_id == $product_label->id ? 'checked' : '' }}/>
...
@push('scripts')
    <script>
        $(document).on('change', '#country-id', function() {
            ...
            $.ajax({
                    type: "POST",
                    url: "{{ route('admin.other.product.get.region.list') }}",
