            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                            {{ __('backend.sidebar.product_label') }}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/product-label/create') }}" class="btn btn-primary " title="Add New Product Label">
                                <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ url('/admin/product-label') }}" id="product-label-form" class="filter-clear-form">
                        <div class="card-header border-0">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.product_label.name') }}</th>
                                    ...
                                    @foreach($productlabel as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/product-label/status-change')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="sticky text-center">
                                                <a href="{{ url('/admin/product-label/' . $item->id . '/edit') }}" title="Edit Product Label">
                                                ...
                                                <form method="POST" action="{{ url('/admin/product-label' . '/' . $item->id) }}" class="deleteForm" style="display: inline;">                
                        ...
                        <div class="row my-3">
                            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                                {{ Admin::tableLength($productlabel) }}
                            </div>
                            <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                                <div class="pagination-wrapper"> {!! $productlabel->appends([
