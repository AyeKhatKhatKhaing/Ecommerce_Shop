<div class="row">
    <div class="col-md-12">
        <h2 class="fs-1">@if($formMode == "edit") {{ __('backend.common.edit') }} @else {{ __('backend.common.create') }} @endif {{ __('backend.sidebar.classification') }}</h2>
    </div>
</div>
<div class="row mt-4">
    <div class="col-md-8">
                                ...
                                <div class="col-md-12">    
                                    <div class="form-group{{ $errors->has('name_'.$lng) ? 'has-error' : ''}}">
                                        @foreach (config('locale.langs_code') as $key => $code)
                                            @if ($lng == $key)
                                                {!! Form::label('name_'.$lng, __('backend.classification.name').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                            @endif
                                        @endforeach
                ...
                <div class="form-group">
                    <div class="form-group">
                        <div class="float-left">
                            ...
                            <button type="button" class="btn btn-secondary cancel" onclick="window.location='{{ url('/admin/classification') }}'"><i class="bi bi-x"
