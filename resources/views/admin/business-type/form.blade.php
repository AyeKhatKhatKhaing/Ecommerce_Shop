<div class="row">
    <div class="col-md-8">
        <h2 class="fs-1">@if($formMode == "edit") {{ __('backend.common.edit') }} @else {{ __('backend.common.create') }} @endif {{ __('backend.sidebar.business_type') }} </h2>
    </div>
                    ...
                    <div class="tab-pane fade {{ $lngcode == 'en' ? 'active show' : '' }}" id="{{$lngcode}}" role="tabpanel">
                        <div class="col-md-12">
                            @foreach (config('locale.langs_code') as $key => $code)
                                @if ($lngcode == $key)
                                    {!! Form::label('name_'.$lng,  __('backend.business_type.name').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                @endif
                            @endforeach
