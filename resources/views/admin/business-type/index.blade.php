        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header border-0"> 
                    <h3 class="card-title">
                        {{ __('backend.business_type.business_type_list') }}
                    </h3>
                    <div class="card-toolbar">
                        <a href="{{ url('/admin/business-types/create') }}" class="btn btn-primary " title="Add New Type">
                            <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                        </a>
                    </div>
                </div>
                <form method="get" action="{{ route('business-types.index') }}" id="business-type-form" class="filter-clear-form">
                    <div class="card-header border-0">
                ...
                <div class="card-body pt-0">
                        ...
                        <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                            <thead>
                                <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                    <th>#</th>
                                    <th>{{ __('backend.business_type.name') }}</th>
                                ...
                                @foreach($types as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                    type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/business-types/status-change')"
                                                    name="status{{ $item->id }}"
                                                    {{ $item->status ? 'checked' : '' }}>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center sticky">
                                            <a href="{{ url('/admin/business-types/' . $item->id . '/edit') }}" title="Edit BusinessType">
                                            ...
                                            {!! Form::open([
                                            'method' => 'DELETE',
                                            'url' => ['/admin/business-types', $item->id],
                    ...
                    <div class="row my-3">
                        <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                            {{ Admin::tableLength($types) }}
                        </div>
                        <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
                            <div class="pagination-wrapper"> {!! $types->appends([
