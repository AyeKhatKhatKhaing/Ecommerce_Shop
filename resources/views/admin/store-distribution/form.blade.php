<div class="row">
    <div class="col-md-8">
        <h2 class="fs-1">@if($formMode == "edit") {{ __('backend.common.edit') }} @else {{ __('backend.common.create') }} @endif {{ __('backend.sidebar.store_distribution') }}</h2>
    </div>
        ...
        <div class="form-group">
            <div class="form-group">
                <div class="float-left">
                    ...
                    <button type="button" class="btn btn-secondary cancel" onclick="window.location='{{ url('/admin/store-distribution') }}'"><i class="bi bi-x"
                    ...
                    <div class="tab-pane fade {{ $lng == 'en' ? 'active show' : '' }}"  id="{{ strtolower($lng) }}-tab">      
                        <div class="form-group {{ $errors->has('banner_title'.$lng) ? 'has-error' : ''}}">
                            @foreach (config('locale.langs_code') as $key => $code)
                                @if ($lng == $key)
                                    {!! Form::label('banner_title_'.$lng, __('backend.store_distribution.banner_title').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                @endif
                            @endforeach
                            {!! Form::text('banner_title_'.$lng, isset($storedistribution) && isset($storedistribution->banner_titles) ? $storedistribution->banner_titles[$lng] : null, ('' == 'required') ? ['class' => 'form-control editor', 'required' => 'required'] : ['class' => 'form-control editor']) !!}
                            {!! $errors->first('banner_title_'.$lng, '<p class="help-block">:message</p>') !!}
                        </div>
                        <div class="form-group mt-4{{ $errors->has('title_'.$lng) ? 'has-error' : ''}}">
                            @foreach (config('locale.langs_code') as $key => $code)
                                @if ($lng == $key)
                                    {!! Form::label('title_'.$lng, __('backend.store_distribution.title').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                @endif
                            @endforeach
                            {!! Form::text('title_'.$lng, isset($storedistribution) && isset($storedistribution->titles) ? $storedistribution->titles[$lng] : null, ('' == 'required') ? ['class' => 'form-control editor', 'required' => 'required'] : ['class' => 'form-control editor']) !!}
                            {!! $errors->first('title_'.$lng, '<p class="help-block">:message</p>') !!}
                        </div>
                        <div class="form-group mt-4{{ $errors->has('description_'.$lng) ? 'has-error' : ''}}">
                            @foreach (config('locale.langs_code') as $key => $code)
                                @if ($lng == $key)
                                    {!! Form::label('description_'.$lng, __('backend.store_distribution.description').'('.$code.')', ['class' => 'control-label mb-3 required']) !!}
                                @endif
                            @endforeach
                            {!! Form::textarea('description_'.$lng, isset($storedistribution) && isset($storedistribution->descriptions) ? $storedistribution->descriptions[$lng] : null, ('' == 'required') ? ['class' => 'form-control editor', 'required' => 'required'] : ['class' => 'form-control editor']) !!}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group{{ $errors->has('meta_title_'.$lng) ? 'has-error' : ''}}">
                                                    @foreach (config('locale.langs_code') as $key => $code)
                                                        ...
                                                        @endif
                                                    @endforeach
                                                    {!! Form::text('meta_title_'.$lng, isset($storedistribution) && isset($storedistribution->meta_titles) ? $storedistribution->meta_titles[$lng] : null, ('' == 'required') ? ['class' => 'form-control', 'required' => 'required'] : ['class' => 'form-control']) !!}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group{{ $errors->has('meta_description_'.$lng) ? 'has-error' : ''}}">
                                                    @foreach (config('locale.langs_code') as $key => $code)
                                                        ...
                                                        @endif
                                                    @endforeach
                                                    {!! Form::textarea('meta_description_'.$lng, isset($storedistribution) && isset($storedistribution->meta_descriptions) ? $storedistribution->meta_descriptions[$lng] : null, ('' == 'required') ? ['class' => 'form-control h-150px', 'required' => 'required'] : ['class' => 'form-control h-150px']) !!}
                                            ...
                                            <div class="card-body pt-0">
                                                ...
                                                <div class="panel-block">
                                                    ...
                                                    <div id="holder-meta-image">
                                                            @if(!empty($storedistribution->meta_image))
                                                                <div class='lfmimage-container meta-imagelfmc0'>
                                                                    <img src="{{ asset($storedistribution->meta_image) }}" class='lfmimage w-100' style="height: 20rem;">
                                                                    <div>
                                                    ...
                                                    <div class="input-group mt-3">
                                                        ...
                                                        <input id="meta-image" class="form-control" type="text" name="meta_image" value="{{isset($storedistribution) ? $storedistribution->meta_image : ''}}">
                                                    </div>
                ...
                <div class="panel-block">
                    <div class="form-group">
                        <div id="holder-banner-image">
                            @if(!empty($storedistribution->banner_image))
                                <div class='lfmimage-container banner-imagelfmc0'>
                                    <img src="{{ asset($storedistribution->banner_image) }}" class='lfmimage w-100' style="height: 20rem;">
                        ...
                        <div class="input-group mt-3">
                            ...
                            <input id="banner-image" class="form-control" type="text" name="banner_image" value="{{isset($storedistribution->banner_image) ? $storedistribution->banner_image : ''}}">
                        </div>
